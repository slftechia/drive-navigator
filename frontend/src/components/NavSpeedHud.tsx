...
export default function NavSpeedHud({ speedKmh, limitKmh, fromOsm = false }: NavSpeedHudProps) {
  const current = speedKmh != null && speedKmh >= 1 ? Math.round(speedKmh) : '—';
  const over = typeof current === 'number' && current > limitKmh + 3;

  return (
    <div className={`nav-speed-hud ${over ? 'nav-speed-over' : ''}`} aria-label="Velocidade">
      <div className={`nav-speed-limit ${fromOsm ? 'nav-speed-limit-osm' : ''}`} title={fromOsm ? 'Limite via OpenStreetMap' : 'Limite estimado'}>
        {limitKmh}
      </div>
